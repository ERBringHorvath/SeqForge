...
*   extract <br/>
    Extract aligned sequences identified via the Genome Seach Query pipeline from original FASTA files.
    Features:
...
*   sanitize<br/>
    *   Remove special characters from input files—**required** for all Genome Seach analyses.
...
**Results:** Directory where Genome Seach results will be stored
...
All Genome Seach results are concatenated to `all_results.csv` and either `all_filtered_results.csv` or <br /> 
