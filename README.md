...
*   extract <br/>
    Extract aligned sequences identified via the SeqFinder Query pipeline from original FASTA files.
    Features:
...
*   sanitize<br/>
    *   Remove special characters from input files—**required** for all SeqFinder analyses.
...
**Results:** Directory where SeqFinder results will be stored
...
All SeqFinder results are concatenated to `all_results.csv` and either `all_filtered_results.csv` or <br /> 
